...
def bhaskara(a, b, c):
    ...
    steps += f"$$x = \\frac{{-b \\pm \\sqrt{{\\Delta}}}}{{2a}}$$\n" \
             f"$$x_1 = \\frac{{-{b} + \\sqrt{{delta}}}}{{2 \\cdot {a}}}$$\n" \
             f"$$x_1 = {x1:.2f}$$\n" \
             f"$$x_2 = \\frac{{-{b} - \\sqrt{{delta}}}}{{2 \\cdot {a}}}$$\n" \
             f"$$x_2 = {x2:.2f}$$\n"
    return (x1, x2), steps
...
operation = st.sidebar.selectbox("Escolha a operação:", [
...
if operation == "Intensidade da Corrente (A)":
    charge = st.sidebar.number_input("Insira a quantidade de carga (C):", step=0.01)
    time = st.sidebar.number_input("Insira o tempo (s):", step=0.01)
    ...
    current = st.sidebar.number_input("Insira a intensidade da corrente (A):", step=0.01)
    time = st.sidebar.number_input("Insira o tempo (s):", step=0.01)
    if st.sidebar.button("Calcular"):
    ...
    a = st.sidebar.number_input("Insira o valor de a:", step=0.01)
    b = st.sidebar.number_input("Insira o valor de b:", step=0.01)
    c = st.sidebar.number_input("Insira o valor de c:", step=0.01)
    if st.sidebar.button("Calcular"):
    ...
    q = st.sidebar.number_input("Insira a carga (C):", step=0.01)
    d = st.sidebar.number_input("Insira a distância (m):", step=0.01)
    if st.sidebar.button("Calcular"):
    ...
    q1 = st.sidebar.number_input("Insira a carga 1 (C):", step=0.01)
    q2 = st.sidebar.number_input("Insira a carga 2 (C):", step=0.01)
    d = st.sidebar.number_input("Insira a distância (m):", step=0.01)
    if st.sidebar.button("Calcular"):
